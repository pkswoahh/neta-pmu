...
- [x] **Ajustes Agenda (feedback QA Roberto):** (1) fix hueco abajo en PWA (media query `display-mode: standalone` → marco 100%, navegador sigue 100dvh); (2) Fecha+Hora lado a lado mismo encuadre, sin `!w-auto`, `lang="es"`; (3) país por defecto del celular derivado de la **moneda** (no del `country` viejo del perfil — arregla el WhatsApp que salía +1); (4) **plantilla de WhatsApp editable** en Configuración (migración 012 `reminder_template`, placeholders {nombre}{procedimiento}{fecha}{hora}{negocio}); (5) aviso al registrar atención de cita futura + procedimiento se guarda con fecha de hoy; (6) tarjetas muestran fecha concreta ("11 jun") además del relativo; (7) estado **"No vino"** (no_show) + acción en el detalle; (8) tarjeta "Citas del periodo" en Dashboard (atendidas/canceladas/no vinieron).
