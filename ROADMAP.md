...
- [x] **Ajustes Agenda (feedback QA Roberto):** (1) fix hueco abajo en PWA (media query `display-mode: standalone` → marco 100%, navegador sigue 100dvh); (2) Fecha+Hora lado a lado mismo encuadre, sin `!w-auto`, `lang="es"`; (3) **país preguntado explícitamente en onboarding** (Nombre → País → Moneda) y editable en Configuración — país y moneda son independientes (ej. Venezuela cobrando en USD); el selector del celular usa `profile.country` (arregla el WhatsApp que salía +1); (4) **plantilla de WhatsApp editable** en Configuración (migración 012 `reminder_template`, placeholders {nombre}{procedimiento}{fecha}{hora}{negocio}); (5) aviso al registrar atención de cita futura + procedimiento se guarda con fecha de hoy; (6) tarjetas muestran fecha concreta ("11 jun") además del relativo; (7) estado **"No vino"** (no_show) + acción en el detalle; (8) tarjeta "Citas del periodo" en Dashboard (atendidas/canceladas/no vinieron).
- [x] **Pulido final pre-comercial (feedback Roberto):** (1) quitada la tarjeta "Próximas citas" del Dashboard (queda solo en Agenda); (2) **pull-to-refresh propio** sobre `<main>` (el bloqueo del teclado había matado el nativo; el propio funciona en navegador y PWA); (3) selector de mes a **todo el ancho** (arreglo del look "mocho"), en Dashboard y Procedimientos; (4) badge "Más vendido" **no aparece en empate** (solo si hay líder claro); (5) **mensaje "No hay datos en este periodo"** cuando el mes/periodo está vacío (antes mostraba tarjetas en cero).
- [x] **Lote pre-comercial 2 (plan-first, feedback Roberto):** Agenda — tarjetas con **día arriba / hora abajo**; vista **Activas / Historial** (interruptor con conteos) + **Reagendar** canceladas/no-vino (vuelven a 'scheduled'); form con **Fecha/Hora apiladas** (sin scroll horizontal). Servicios y Gastos — barra en **2 filas** (chips arriba; navegador de periodo + CSV + '+' abajo) vía PeriodSelector; **un solo buscador** (quitado el desplegable de tipo, el buscador ya filtra por tipo/categoría). Clientes — buscar por **nombre o celular**. Configuración — botón **'+' dinámico** (gris apagado vacío → rosado vivo al escribir).
- [x] **Bugs QA (lote 3):** pull-to-refresh menos sensible (zona muerta + umbral más alto); fix desborde de `<input date/time>` en iOS (globals: min-width/max-width); botón "Nuevo" más prominente (con label) en Servicios/Gastos; listas de Servicios/Gastos muestran **fecha concreta** (dd/mm/yyyy) en vez de "Hace N días"; **mensaje de recordatorio más profesional** (sin emojis ni "cualquier cosa me escribes"); **nombre del negocio en el header** móvil junto al logo; **teléfonos en datos demo** (el detalle/WhatsApp/búsqueda por celular ya se ven con datos).
- [x] **Scroll nativo definitivo (rediseño):** se quitó el bloqueo de scroll del documento y el pull-to-refresh propio. Ahora el **documento scrollea nativo** → vuelven el pull-to-refresh y el rebote del navegador. El bottom nav pasó a `position: fixed` y se **oculta mientras el teclado está abierto**, detectándolo con `visualViewport` (robusto, cubre el botón "esconder teclado" de Chrome). Resuelve: PTR sensible, scroll tieso, y el bug original del teclado de forma más sólida. Fix real del desborde de `<input date/time>` en iOS con `appearance: none`. `AppLayout.tsx` + `globals.css`. **OJO:** re-probar teclado en Chrome/Safari/PWA.
- [ ] **Pendiente menor:** el calendario nativo de `<input type=date>` en iOS usa el idioma del **dispositivo**, no de la app. Si molesta, evaluar date picker propio en español (no MVP).
